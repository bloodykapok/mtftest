...
//本机配置信息
uint8_t ResourceManager_dest_ip[16];
uint8_t higherups[10];

//模块中数据包处理占用标识（当模块中存在数据包占用处理线程期间，对其它进入的数据包做直接丢包处理）
int ResourceManager_PlaneOccupy; //ResourceManager模块转发平面占用标识
int ResourceManager_UpOccupy;    //ResourceManager模块上行通道占用标识
int ResourceManager_DownOccupy;  //ResourceManager模块下行通道占用标识

/*

	//第一时间检查ResourceManager模块上行通道中是否有其它数据包占用，如果有，直接丢包；如果没有，启动占用
	if(ResourceManager_UpOccupy == 1)
	{
		continue;
	}
	else
	{
		ResourceManager_UpOccupy = 1;
	}

	//第一时间检查ResourceManager模块下行通道中是否有其它数据包占用，如果有，直接丢包；如果没有，启动占用
	if(ResourceManager_DownOccupy == 1)
	{
		continue;
	}
	else
	{
		ResourceManager_DownOccupy = 1;
	}

	//对ResourceManager模块上行通道解除占用
	ResourceManager_UpOccupy = 0;

	//对ResourceManager模块下行通道解除占用
	ResourceManager_DownOccupy = 0;
