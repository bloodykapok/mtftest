...
//本机配置信息
uint8_t CacheRouter_dest_ip[16];

//模块中数据包处理占用标识（当模块中存在数据包占用处理线程期间，对其它进入的数据包做直接丢包处理）
int CacheRouter_PlaneOccupy; //CacheRouter模块转发平面占用标识
int CacheRouter_UpOccupy;    //CacheRouter模块上行通道占用标识
int CacheRouter_DownOccupy;  //CacheRouter模块下行通道占用标识

/*

	//第一时间检查CacheRouter模块上行通道中是否有其它数据包占用，如果有，直接丢包；如果没有，启动占用
	if(CacheRouter_UpOccupy == 1)
	{
		continue;
	}
	else
	{
		CacheRouter_UpOccupy = 1;
	}

	//第一时间检查CacheRouter模块下行通道中是否有其它数据包占用，如果有，直接丢包；如果没有，启动占用
	if(CacheRouter_DownOccupy == 1)
	{
		continue;
	}
	else
	{
		CacheRouter_DownOccupy = 1;
	}

	//对CacheRouter模块上行通道解除占用
	CacheRouter_UpOccupy = 0;

	//对CacheRouter模块下行通道解除占用
	CacheRouter_DownOccupy = 0;
